...
class TrainingLogCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if self.step_count % 1000 == 0:
            ...
            print(f"Логирование на шаге {self.num_timesteps}: Mean Reward = {mean_reward}, Fires Left = {fires_left}")
        return True
    ...
    for step in range(max_steps):
        ...
        if done:
            if len(test_env.fires) == 0:
                ...
                print(f"Тестирование завершено на шаге {step + 1}: Достигнут лимит шагов. Общая награда: {total_reward}")
            else:
...
def train_and_evaluate(fire_count, obstacle_count):
    ...
    model.learn(total_timesteps=500000, callback=[eval_callback, training_log_callback], progress_bar=True)
